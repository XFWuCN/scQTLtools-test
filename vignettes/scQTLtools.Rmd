...
![scQTLtools overview](Overview.svg){width=90%}
...
column is one sample, and the scoring method is 0/1/2/3, o represents missing
...
result data frames, and slots for biClassify, species, and group information.

**Example**

```{r createObject, message=FALSE}
...
eqtl  <- normalizeGene(eQTLObject = eqtl, method = "logNormalize")
```
...
They can also use `upstream` and `downstream` to specify SNPs proximal to the gene in the genome.

**Example**

```{r eQTLsc3, message=FALSE}
 eqtl3 <- callQTL(eqtl,
                  gene_ids = NULL,
                  downstream = -85000000,
                  upstream = 20000000,
...
Here we use `visualizeQTL` to visualize the result. There are four types of plots available to visualize sc-eQTL results. Users can choose "histplot", "violin", "boxplot", or "QTLplot". "In addition, the parameter `groupName` is used to specify a particular single-cell group of interest."

**Example**

```{r visualizeQTL, message=FALSE}
visualizeQTL(eQTLObject = eqtl1,
             SNPid = "1:632647",
             Geneid = "RPS27",
             groupName = NULL,
             plottype = "QTLplot",
